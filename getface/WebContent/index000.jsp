...
%>
  <div class="demo-title">
    <p>人脸识别例子，注意：本站人脸识别接口供国内程序员和公司体验快速高效的人脸识别，提供的接口注册和使用完全免费。关于本例子的使用请点击“例子说明”按钮查看，可以点击“源码下载”下载本例子的源码。</p>
  </div>
  <!-- 用于显示视频 -->
  <div class="demo-frame" id="cap-frame" class="white_content">
    <div class="demo-container">
      ...
      <button id="begindetect" onclick="begindetect()">人脸识别</button>
      <p><label></label>&nbsp<p>
      <p><label></label>&nbsp<p>
      <button id="beginsearch" onclick="beginsearch()">人脸搜索</button>
      <p><label></label>&nbsp<p>
      <p><label></label>&nbsp<p>
      ...
      <p><label></label>&nbsp<p>
      <p><label></label>&nbsp<p>
      <button id="downloadsource" onclick="downloadsource()">源码下载</button>
      <p><label></label>&nbsp<p>
  ...
  <div style="height:300px;width:800px;text-align:center;top:0;left:0;bottom:0;right:0;position: absolute;margin: auto;">
  <p>本例子是特征提取接口、人脸识别和人脸搜索接口的例子（调用例子可以下载源码自行观看，这里科普一下，特征提取是指人脸特征提取，就像人一样，只有见过了，才认识嘛，这个接口的作用就是这个；人脸识别是指同时传入需要校验的人脸的照片和之前添加的特征识别字符，然后接口判断传入的人脸照片和识别字符跟人脸库里面的记录是否一致，这接口通常用于登录验证，身份确认等，人脸搜索是指不知人脸身份，只传入图片数据，然后接口从图片中找出人脸并和人脸库比对，返回之前添加的人脸识别字符串），建议在电脑上运行此例子，使用一般的摄像头（经过实测30万实际像素以上就可以了，当然基于提高识别速度的原因，建议使用80万实际像素以上的摄像头）即可，其他接口的使用，请自行根据文档来调用，在某些手机上使用有时视频会变成横向，调起来比较繁琐，而这只是一个例子也就不费这个劲了，自行换手机看吧。在电脑上谷歌浏览器（Chrome）和火狐浏览器（Firefox）测试通过，在手机上UC浏览器测试通过，其余浏览器没有试过，亲们可以自行试一下。接口免费调用，调用方法请点击“接口文档”获取
  <p><label></label>&nbsp<p>
    		  ...
    		  if(captyle=="搜索人脸"){
	    		  ...
	    		  $.ajax({
	                  ...
	                  success: function(data) {
	                      ...
	                      //解析webservice的返回值
	                	  var xmlDoc=data;
	                	  var returns = xmlDoc.getElementsByTagName('serachfacesReturn');
